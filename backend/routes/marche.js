...
router.delete("/:id", (req, res) => {
  ...
  db.get(
    "SELECT COUNT(*) as count FROM modelli WHERE marche_id = ?",
    [id],
    (err, row) => {
      if (err) {
      ...
      if (modelliCount > 0) {
        console.log(
          `Tentativo di eliminare marca ID ${id} con ${modelliCount} modelli - BLOCCATO`,
        );
      ...
      db.get(
        "SELECT COUNT(*) as count FROM ordini WHERE marca_id = ?",
        [id],
        (err2, row2) => {
          if (err2) {
          ...
          if (ordiniCount > 0) {
            console.log(
              `Tentativo di eliminare marca ID ${id} con ${ordiniCount} ordini - BLOCCATO`,
            );
            return res.status(400).json({
              error:
                ordiniCount === 1
