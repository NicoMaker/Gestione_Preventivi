...
const DB_PATH = path.join(__dirname, "..", "db", "Preventivi.db");
  ...
  return new Promise((resolve, reject) => {
    db.serialize(async () => {
      try {
        ...
        console.log(
          "  ✓ Tabella ordini OK (marca e modello obbligatori, con contratto_finito)",
        );
          ...
          (err) => {
            if (err) {
              if (err.message.includes("duplicate column")) {
                ...
                console.error(
                  "  ✗ Errore migrazione contratto_finito:",
                  err.message,
                );
              }
            } else {
              console.log(
                "  ✓ Colonna contratto_finito aggiunta (record esistenti = 0)",
              );
            }
          },
        );
...
async function seedUtenti() {
  ...
  for (const utente of utenti) {
    try {
      ...
      utentiIds.push(id);
      console.log(
        `  ✓ Utente: ${utente.nome} / ${utente.password} (ID: ${id})`,
      );
    } catch (err) {
...
async function seedMarche() {
  ...
  for (const marca of marche) {
    try {
      const id = await runQuery("INSERT INTO marche (nome) VALUES (?)", [
        marca,
      ]);
      marcheIds[marca] = id;
      ...
      if (err.message.includes("UNIQUE")) {
        ...
        const existing = await getQuery(
          "SELECT id FROM marche WHERE nome = ?",
          [marca],
        );
...
async function seedModelli(marcheIds) {
  ...
  for (const modello of modelli) {
    if (!modello.marche_id) {
      console.error(
        `  ✗ ERRORE: Modello "${modello.nome}" non ha marca associata! SALTATO.`,
      );
      errorCount++;
      continue;
    }
    ...
    try {
      ...
      console.log(
        `  ✓ Modello: ${modello.nome} (ID: ${id}, Marca ID: ${modello.marche_id})`,
      );
    } catch (err) {
      if (err.message.includes("UNIQUE")) {
        ...
        console.error(
          `  ✗ ERRORE: Modello "${modello.nome}" manca marca_id (NOT NULL violation)`,
        );
        errorCount++;
      } else {
...
async function seedClienti() {
  ...
  const clienti = [
    ...
    },
    {
      nome: "Elena Gallo",
      num_tel: "3401234567",
      email: "elena.gallo@email.it",
      data_passaggio: "2024-12-20",
      flag_ricontatto: 1,
    },
    {
    ...
    },
    {
      nome: "Laura De Luca",
      num_tel: "3473456789",
      email: "laura.deluca@gmail.com",
      data_passaggio: "2024-06-18",
      flag_ricontatto: 0,
    },
    {
      nome: "Federica Piras",
    ...
    },
    {
      ...
      num_tel: "3358901234",
      email: "riccardo.vitale@gmail.com",
      data_passaggio: null,
      flag_ricontatto: 0,
    },
  ];
...
async function seedOrdini(clientiIds, modelliIds, marcheIds) {
  ...
  console.log(
    "  ⚠️  IMPORTANTE: marca_id e modello_id sono entrambi obbligatori (NOT NULL)",
  );
  ...
  for (const ordine of ordini) {
    ...
    if (!ordine.marca_id) {
      console.error(
        `  ✗ Preventivo SALTATO: marca_id obbligatorio! (cliente_id: ${ordine.cliente_id})`,
      );
      countSaltati++;
      continue;
    }
    if (!ordine.modello_id) {
      console.error(
        `  ✗ Preventivo SALTATO: modello_id obbligatorio! (cliente_id: ${ordine.cliente_id})`,
      );
      countSaltati++;
      continue;
    }
    ...
    try {
      ...
      const clienteNome =
        clientiIds.find((c) => c.id === ordine.cliente_id)?.nome || "N/A";
  ...
  if (countSaltati > 0) {
    console.warn(
      `  ⚠️  Saltati (campo obbligatorio mancante): ${countSaltati}`,
    );
  }
  return ordiniIds;
}
...
async function seedDatabase() {
  try {
    ...
    console.log(
      `  • Modelli:    ${modelliIds.length} (tutti con marca obbligatoria)`,
    );
    console.log(`  • Clienti:    ${clientiIds.length}`);
    console.log(
      `      📱 Solo cellulare: ${clientiIds.filter((c) => c.num_tel && !c.email).length}`,
    );
    console.log(
      `      📧 Solo email:     ${clientiIds.filter((c) => !c.num_tel && c.email).length}`,
    );
    console.log(
    ...
    console.log(
      "⚠️  Tutti i preventivi hanno marca E modello obbligatori (NOT NULL)",
    );
    console.log(
